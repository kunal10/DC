    ...
    while (getCurrentTimeSecs() < config.getStopTime()) {
      ...
      try {
        ...
        if (request == null) {
          ...
          System.out
                  .println("Received broadcast request: " + request.toString());
...
public class BroadcastServer {
  public BroadcastServer(Config config) {
    ...
    Comparator<Message> messageDeliveryComparator =
            new Message.MessageDeliveryComparator();
  ...
  public void start() throws IOException {
    ...
    System.out.println("Started a new Broadcast Server on port: "
    ...
    // Wait for 2 sec before shut down the server.
    // while (getCurrentTimeSecs() < config.getStopTime() + 2) {}
    // serverSocket.close();
